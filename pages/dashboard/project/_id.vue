        ...
        <div class="wrap_my_form xydesx">
          <div class="project_header">
            ...
            <div class="project_header_action">
              <nuxt-link
                :to="{ path: '/dashboard/project/edit/' + project_id }"
              >
                <svg
                  width="22"
                  height="22"
          ...
          <div class="project_body container-fluid">
            <div class="project_task row">
              <div class="project_task_graph col-lg-2"></div>
              <div class="project_task_notification col-lg-2"></div>
              <div class="project_task_right col-lg-8">
                <div class="project_warning"></div>
              </div>
            </div>
          </div>
          <div class="project_task_table">
            <div class="tacker_tag">
              <p>
                Task Tracker
              </p>
              <nuxt-link
                :to="{ path: '/dashboard/task/create/' + project_id }"
                tag="button"
            ...
            <table class="project_task_tracker">
              ...
              <tbody>
                <tr v-for="data in taskData" :key="taskData.id">
                  ...
                  <td>
                    <span>{{
                      data.assignto ? data.assignto.first_name : "assign_to"
                    }}</span>
                  </td>
                  <td>
  ...
  methods: {
    deleteTask($id){
        ...
        if (result.value) {
          try {
            this.$axios
              .delete("auth/deleteTask/" + $id)
              .then(({ data }) => {
                
                this.$swal("Deleted!", data.message, "success");
                this.getTaskProjectRecord();
              });
          } catch (e) {}
    ...
    deleteProject() {
      this.$swal({
        ...
        if (result.value) {
          try {
            this.$axios
              .delete("auth/deleteProject/" + this.project_id)
              .then(({ data }) => {
    ...
    async getTaskProjectRecord() {
      this.project_id = this.$route.params.id;
      try {
        await this.$axios
          .get("auth/getTaskProjectRecord/" + this.project_id)
          .then(({ data }) => {
            console.log(data);
            this.taskData = data;
            // this.preloader = false;
    ...
    async getDefault() {
      await this.getProjectRecord();
      await this.getTaskProjectRecord();
      this.preloader = false;
    }
  },
  mounted() {
